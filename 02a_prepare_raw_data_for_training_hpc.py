...
def convert_tif_to_hdr(input_img: str, output_dir: str) -> None:
    """
    Use ImageJ and the convert_tif_to_img macro script to convert .tif files to .hdr & .img files

    :param input_img: path to the images
    :param output_dir: path to the folder in which the output should be saved in
    :return:
    """
    # os.makedirs(output_dir, exist_ok=True)
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    print("Opening ImageJ to convert .tif to .hdr files")
    subprocess.Popen(
        rf'{PATH_ImageJ} --headless -macro convert_tif_to_img_cluster "{input_img}--{output_dir}"',
        #fr'{PATH_ImageJ} -macro convert_mha_to_img "{input_dir}--{output_dir}"', # it helps to put it in macro mode to debug
        shell=True,
    ).wait()
...
if __name__ == "__main__":
    ...
    command line arguments for input directories and file names, rather than hardcoding them. 
    To launch the script, please use the bash file 01a_prepare_raw_data.sh which is located in the "Utilities" folder.
    
    With this script, each image and its corresponding annotation can be processed and saved in its individual 
    temporary folder, which is then deleted after processing. This is more efficient for cluster processing
    ...
    parser.add_argument('-im', type=Path, required=True, help='Path to the input directory')
    parser.add_argument('-an', type=Path, required=True, help='Path to annotation files')
    parser.add_argument('-id', type=Path, required=True, help='File name of image')
    ...
    Manage Folders
    """
    temp_img_folder = join(output_folder, f"temp_images_nii_{args.id}") 
    temp_mask_folder = join(output_folder, f"temp_labels_nii_{args.id}") 
    ...
    convert_tif_to_hdr(args.an, temp_mask_folder) # with the new napari workflow for annotations, the images are saved in .tif automatically
    ...
    convert_tif_to_hdr(args.im, temp_img_folder)  # do not need to replace it because it is read from path.py and has the same name as masks
    ...
    For the cluster version, this step is now carried out afterwards with a script of his own (see generate_dataset_json.py)
    """
    ...
    """
    Step5: Clean up and delete temporary folders
    """
